...
class LinkedInSearchScraper:
    ...
    def is_us_only(self, job: dict) -> bool:
        """Returns True if job is US-only (not valid for Mexico-based worker)"""
        location = (job.get("location") or job.get("Location") or "").lower()
        title = (job.get("title") or job.get("Role") or "").lower()

        # Check US-only phrases in location
        for phrase in US_ONLY_PHRASES:
            if phrase in location:
                return True

        # If location contains a US state name AND doesn't mention LATAM/Mexico/remote-global
        us_states = [
            ...
            ", ca", ", ny", ", tx", ", fl", ", il", ", wa", ", co", ", ga", ", ma",
            "united states",
        ]

        latam_hints = ["latam", "latin america", "mexico", "méxico", "global", "worldwide", "anywhere"]
        has_latam = any(h in location for h in latam_hints)
