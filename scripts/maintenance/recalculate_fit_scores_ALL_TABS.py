#!/usr/bin/env python3
"""
AI JOB FOUNDRY - Recalculate FIT Scores with Salary Weight (ALL TABS)
Recalcula FIT scores considerando salario como factor crítico
✅ NUEVA VERSIÓN: Procesa TODAS las pestañas automáticamente
"""
import sys
import os
import re
...
def process_worksheet(worksheet, tab_name):
    ...
    try:
        ...
        print(f"  📊 Found {len(data_rows)} jobs in '{tab_name}'")
        ...
        for idx, row in enumerate(data_rows, start=2):
            ...
            if new_score != current_score:
                try:
                    ...
                    print(f"      ✅ Row {idx}: {current_score} → {new_score}/10 (Salary: ${salary_mxn:,.0f} MXN)")
                    updated_count += 1
...
def recalculate_fit_scores():
    """Recalculate FIT scores for ALL tabs"""
    print("\n💰 Recalculating FIT Scores with Salary Weight (ALL TABS)...")
    print("="*70)
    ...
    # ✅ NUEVO: Procesar TODAS las pestañas
    all_worksheets = spreadsheet.worksheets()
    print(f"\n📑 Found {len(all_worksheets)} tabs in spreadsheet")
    ...
    for worksheet in all_worksheets:
        ...
        # ⚠️ Skip non-job tabs
        if tab_name.lower() in ['config', 'settings', 'dashboard', 'summary']:
            print(f"\n  ⏭️  Skipping '{tab_name}' (config tab)")
            continue
        
        print(f"\n📋 Processing '{tab_name}'...")
        updated = process_worksheet(worksheet, tab_name)
        
        if updated > 0:
            print(f"  ✅ Updated {updated} FIT scores in '{tab_name}'")
    ...
    print(f"   Total FIT scores updated: {total_updated}")
    print("="*70 + "\n")
