"""
INDEED SMART VERIFIER - Verifica jobs de Indeed leyendo REALMENTE el contenido
Usa Playwright para detección precisa de jobs expirados
"""
import os
import sys
...
class IndeedSmartVerifier:
    def __init__(self):
        self.manager = SheetManager()
        self.spreadsheet_id = os.getenv("GOOGLE_SHEETS_ID")
        
        # Textos que indican job expirado en Indeed
        self.expired_indicators = [
            ...
            "la publicación expiró",
            "este empleo ya no está disponible"
        ]
        
        # Textos que indican job ACTIVO en Indeed
    ...
    def get_jobs_to_verify(self, status_filter="New", limit=None):
        """
        Obtiene jobs de Indeed para verificar
        """
        result = self.manager.service.spreadsheets().values().get(
            spreadsheetId=self.spreadsheet_id,
            range="Indeed!A1:Z10000"
        ).execute()
    ...
    def verify_single_job(self, url, browser):
        """
        Verifica si un job de Indeed está activo o expirado
        """
        try:
            ...
            try:
                ...
                if any(word in title for word in ['error', '404', 'not found', 'expired']):
                    return {
                        'status': 'EXPIRED',
    ...
    def mark_job_status(self, row, new_status, reason):
        """
        Marca el status de un job en Google Sheets (Indeed tab)
        """
        # Get headers
        result = self.manager.service.spreadsheets().values().get(
            spreadsheetId=self.spreadsheet_id,
        ...
        # Update Status
        status_cell = f"Indeed!{chr(65 + status_idx)}{row}"
        ...
        if why_idx >= 0:
            why_cell = f"Indeed!{chr(65 + why_idx)}{row}"
            self.manager.service.spreadsheets().values().update(
                spreadsheetId=self.spreadsheet_id,
                range=why_cell,
                valueInputOption='RAW',
    ...
    def verify_all(self, limit=None, mark_expired=True, delete_expired=False):
        """
        Verifica TODOS los jobs de Indeed
        """
        print("\n" + "="*70)
        print("🔍 INDEED SMART VERIFIER")
        print("="*70)
        ...
        # Get jobs
        print("📋 Fetching jobs from Indeed tab...")
